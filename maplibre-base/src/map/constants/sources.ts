const arcgis_hybrid = "http://37.60.239.85:7070/arcgis_hybrid.json";

const google_streets = "http://37.60.239.85:7070/google_streets.json";

const google_satelite = "http://37.60.239.85:7070/google_satelite.json";

const google_satelite_hybrid = "http://37.60.239.85:7070/google_satelite_hybrid.json";

const openStreetMap = "https://raw.githubusercontent.com/go2garret/maps/main/src/assets/json/openStreetMap.json";

const darkMatter = "https://basemaps.cartocdn.com/gl/dark-matter-gl-style/style.json";

const positron = "https://basemaps.cartocdn.com/gl/positron-gl-style/style.json";

const voyager = "https://basemaps.cartocdn.com/gl/voyager-gl-style/style.json";

export {
    arcgis_hybrid,
    google_streets,
    google_satelite,
    google_satelite_hybrid,
    openStreetMap,
    darkMatter,
    positron,
    voyager,
}